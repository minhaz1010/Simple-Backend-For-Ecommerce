...
export const productValidationSchema = z.object({
  ...
  description: z.string({ required_error: "Product description must be provided" }),
  price: z.number({ invalid_type_error: "Price must be a number" }).positive({ message: "Price must be greater than 0" }),
  category: z.string({ required_error: "Product must have a category" }),
  tags: z.string().array().min(1, { message: "At least one tag is required for each product" }),
  variants: variantsValidationSchema.array().nonempty({ message: "At least one variant is required" }),
