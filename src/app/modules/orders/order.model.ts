...
orderSchema.pre("save", async function(next) {
  ...
  // INFO: if orderQuantity is greater than then product quantity then it will throw an error
  ...
  if (checkProductQuantity > 0) {  // NOTE: if product quantity > 0 then quantity will be subtracted from order quantity
  ...
  } else {  // NOTE: if prdcut quantity < 0 then quantity will be 0 and inStock will be false 
    await Product.findOneAndUpdate(
      { _id: productid },
      {
